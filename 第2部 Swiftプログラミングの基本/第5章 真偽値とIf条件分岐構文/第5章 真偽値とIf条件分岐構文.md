...
「空腹と満腹」や「雨と晴れ」、あるいは「有罪と無罪」などのように、ある事象を相反する2つの状態だけで表現することはとても画期的な発明です。
...
Swiftの真偽値のリテラルは「`true`と`false`」のふたつだけです。
...
これらの値は`Bool`型として定義されています。
\
\
実際のプログラムにおいて、`Bool`値は頻繁に使用されます。

次のコードは、整数の`8`が「指定した数で割り切れるかどうか」を調べます。
...
したがって、「`8`が`2`の倍数かどうか」を評価すると`true`を返します。
同様に、「`8`が`3`の倍数かどうか」を評価すると`false`を返します。

+++

他のプログラミング言語では真偽値を`0`と`1`で代用できる場合もあります。
ただし、Swiftは意図しない演算を防ぐために、真偽値は明確に`true`と`false`だけで表現します。
