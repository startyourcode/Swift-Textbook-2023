...
Swiftの論理演算は、論理積と論理和の他に**論理否定**があります。
論理否定は`Bool`値の真偽を反転する演算です。
つまり、`true`の論理否定は`false`、`false`の論理否定は`true`になります。

Swiftプログラミングでは疑問符の論理否定演算子`!`を使って、これを実行できます。
論理否定演算子は「データの直前」に記述する、いわゆる前置き演算子です。

```swift
!true   // false; This means not true
!false  // true; This means not false
```

上のコードについて、論理否定演算子`!`を英語の*not*に置き換えると1行目のコードは`not true`、2行目のコードは`not false`と読むことができます。
...
このifステートメントは、「正しいパスコードの入力」か「FaceIDの認証」ができるなら、iPhoneのロックを解除します。
つまり、iPhoneのロックを解除できないのは、「パスコードが正しくないこと」と「FaceIDが認証できないこと」を同時に発生した場合だけです。
これを言い換えると、「パスコードが正しくないこと」と「FaceIDが認証できないこと」が同時に起こらなければ、iPhoneのロックは解除できると理解できます。

論理否定演算子`!`を使うと、上のifステートメントを次のように書き直すことができます。
...
「論理積で連結されたふたつの否定」がさらに否定されているので、コードは著しく読みづらくなってしまいました。
このような場合、論理式を説明する変数をあらかじめ宣言しておくと、コードの読みやすさを改善できます。

以下に定義する定数`locked`は、ふたつの真偽値が両方とも`false`になった場合にだけ`true`が設定されます。
つまり、「パスコードが正しくないこと」と「FaceIDが認証できないこと」が同時に起こったら、iPhoneがロックされることを示します。

```swift
let locked = !enterdPasscode && !passedFaceID
```

この説明変数`locked`を使って、以下のように「iPhoneのロックを解除する条件分岐プログラム」を書き直せます。
...
このifステートメントは「*If not locked, iPhone is unlocked."*」と、文章のように読むことができます。
最初のifステートメントと同じく、2つの条件が両方とも`false`でない限り、iPhoneのロックは解除されます。
\
\
...
読みやすくて簡潔なコードを記述することは、プログラミングの重要なスキルです。
ここでは、単純なifステートメントをより簡潔に記述する方法を解説します。

次のifステートメントは「天気が良ければお出かけして、そうでないなら家で過ごす」ための条件分岐プログラムです。
定数`isFine`は「天気が良いかどうか」を示しています。
...
三項条件演算子という名前が示す通り、「条件」と「真の値」および「偽の値」という3つの項で形成される演算子です。
三項条件演算子は、疑問符`?`とコロン記号`:`を組み合わせて記述します。

上のifステートメントは三項条件演算子を使って、以下のように記述できます。
三項条件演算子の疑問符`?`の前後には空白スペースがある点に注目してください。

```swift
print(isFine ? "Let's go out!" : "Stay home.")   // Let's go out!
```

このコードは、定数`isFine`が`true`なら`"Let's go out!"`、そうでないなら`"Stay home."`を出力します。

+++

三項演算子は特殊な構文に見えますが、とても有用な手法です。
実際のプログラミングでも、単純なifステートメントは三項条件演算子で記述することを検討してみると良いでしょう。
